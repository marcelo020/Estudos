...
let themeMode = localStorage.getItem('theme-preference') || 'light';
...
function normalizeCandidate(candidate = {}) {
  ...
  return {
    ...
    matrizScores: candidate.matrizScores && typeof candidate.matrizScores === 'object' ? { ...candidate.matrizScores } : {},
...
const MATRIZ_WEIGHTS = {1:3, 2:3, 3:3, 4:4, 5:3, 6:2, 7:4, 8:5, 9:2, 10:3};
const MATRIZ_MAX_SCORE = 160;

function calcMatrizScore(matrizScores) {
  if (!matrizScores || !Object.keys(matrizScores).length) return null;
  let total = 0;
  Object.entries(matrizScores).forEach(([id, score]) => {
    total += (MATRIZ_WEIGHTS[parseInt(id)] || 0) * score;
  });
  return total;
}
...
function renderTable() {
  ...
  dados.forEach(c => {
    ...
    const matrizEvaluated = c.matrizScores && Object.keys(c.matrizScores).length > 0;
    const matrizScore = matrizEvaluated ? calcMatrizScore(c.matrizScores) : null;
    const matrizPercent = matrizScore !== null ? Math.round((matrizScore / MATRIZ_MAX_SCORE) * 100) : null;
    ...
    row.innerHTML = `
      ...
      <td>
        <span class="badge ${statusCandidato === 'Apto' ? 'apto' : 'pendente'}">${statusCandidato}</span>
        ${matrizScore !== null ? `<span class="matriz-score-badge" title="Pontuação na matriz avaliativa">${matrizScore}/${MATRIZ_MAX_SCORE} &middot; ${matrizPercent}%</span>` : ''}
      </td>
      <td>
        <div class="actions-cell">
          ...
          <a class="mini-btn" href="matriz.html?id=${c.id}">${matrizEvaluated ? '✓ Matriz' : 'Avaliar matriz'}</a>
        </div>
      </td>
    `;
...
function saveCandidate(event) {
  ...
  const payload = {
    ...
    matrizScores: existingCandidate?.matrizScores || {},
    pendencias: avaliacao.pendencias,
    status: avaliacao.status,
    pendenciaCritica: avaliacao.pendenciaCritica
  };
...
campos.nome.addEventListener('blur', () => { if (!campos.nome.value.trim()) setFieldValidity(campos.nome, false); });
campos.cpf.addEventListener('blur', () => { if (campos.cpf.value && !validarCPF(campos.cpf.value)) setFieldValidity(campos.cpf, false); });
campos.email.addEventListener('blur', () => { if (campos.email.value && !isValidEmail(campos.email.value)) setFieldValidity(campos.email, false); });
campos.telefone.addEventListener('blur', () => { if (campos.telefone.value && !isValidPhone(campos.telefone.value)) setFieldValidity(campos.telefone, false); });
campos.area.addEventListener('blur', () => { if (!campos.area.value) setFieldValidity(campos.area, false); });
...
if (currentUser) {
  ...
  const validTabs = ['dashboard', 'cadastro', 'lista', 'projeto-ia'];
  setActiveTab(validTabs.includes(hashTab) ? hashTab : 'dashboard', false);
  refresh();
  restoreFormDraft();

  // Mostrar informações do usuário
  const roleLabel = currentUser.role === 'candidato' ? 'Candidato' : 'Responsável';
  userInfoEl.innerHTML = `<span>👤 ${escapeHTML(currentUser.name)}</span><span style="opacity:0.6; font-size:var(--text-xs);">${roleLabel}</span>`;
